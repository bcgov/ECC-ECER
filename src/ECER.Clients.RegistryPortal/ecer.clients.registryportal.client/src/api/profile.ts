...
const putProfile = async (
  ...
  return apiResultHandler.execute({
    request: client.profile_put({}, user),
    key: "profile_put",
  });
};

const postProfileVerification = async (
  identification: Components.Schemas.ProfileIdentification,
): Promise<ApiResponse<any>> => {
  const client = await getClient();
  return apiResultHandler.execute({
    request: client.profileVerification_post({}, identification),
    key: "profileVerification_post",
  });
};
