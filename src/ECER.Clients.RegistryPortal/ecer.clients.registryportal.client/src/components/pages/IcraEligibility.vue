            ...
            <v-row>
              <v-col>
                ...
                <v-btn
                  ...
                  :loading="
                    loadingStore.isLoading('icra_put') ||
                    loadingStore.isLoading('icra_post')
                  "
                  @click="handleSubmit"
                >
                  Submit
...
export default defineComponent({
  ...
  setup: async () => {
    ...
    //Initialize wizard
    await wizardStore.initializeWizardForIcraEligibility(
      applicationWizardIcraEligibility,
      icraStore.draftIcraEligibility,
    );
  ...
  computed: {
    ...
    showSaveButtons() {
      return (
        ...
        !(
          this.wizardStore.currentStepStage === "EmploymentExperience" &&
          this.wizardStore.listComponentMode === "add"
        ) &&
        !(
          this.wizardStore.currentStepStage === "InternationalCertification" &&
          this.wizardStore.listComponentMode === "add"
        )
      );
    },
  ...
  methods: {
    async handleSubmit() {
      const submitIcraEligibilityResponse =
        await this.icraStore.submitIcraEligibilityApplication();

      if (submitIcraEligibilityResponse?.eligibility) {
        if (
          submitIcraEligibilityResponse !== null &&
          submitIcraEligibilityResponse !== undefined &&
          submitIcraEligibilityResponse.eligibility
        ) {
          this.icraStore.icraEligibility =
            submitIcraEligibilityResponse.eligibility;
        }
        this.router.push({
          name: "icra-eligibility-submitted",
          params: {
            icraEligibilityId: submitIcraEligibilityResponse.eligibility.id,
          },
        });
      }
    },
    ...
    incrementWizard() {
      ...
      this.icraStore.draftIcraEligibility.portalStage = this.wizardStore
        .currentStepStage as IcraEligibilityStage;
    },
    decrementWizard() {
    ...
    async saveDraftAndAlertSuccess(exit: boolean) {
      ...
      if (draftIcraEligibilityResponse?.eligibility) {
        let message =
          "Information saved. If you save and exit, you can resume your eligibility submission later.";
        if (exit)
          message =
            "Information saved. You can resume your eligibility submission later.";
