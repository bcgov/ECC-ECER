<template>
  <v-sheet :rounded="0" flat color="primary" width="100%">
    <PageContainer :margin-top="false" class="py-13">
      <h1 class="text-white">
        Welcome to the Early Childhood Educator (ECE) Registry
      </h1>
      <p class="small text-white mt-4">
        Manage your certification to work in a licensed child care facility in
        B.C.
      </p>
    </PageContainer>
  </v-sheet>
  <PageContainer :margin-top="false">
    <div v-for="systemMessage in configStore.systemMessages">
      <div
        v-if="
          systemMessage.portalTags && systemMessage.portalTags.includes('LOGIN')
        "
    ...
    <v-row>
      <v-col cols="12" md="6" class="mb-12">
        <div class="d-flex flex-column ga-4">
          ...
          <p>
            You need to create an account to submit a new application or access
