<template>
  <ReferencePreviewCard
    :is-valid="true"
    title="Work experience hours of applicant"
  ...
  >
    <template #content>
      ...
      <v-row>
        <v-col cols="4">
          <p class="small">
            Where did the applicant complete their work experience hours?
          </p>
        </v-col>
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.childrenProgramName }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="4">
          <p class="small">
            What was their role while completing the work experience hours?
          </p>
        </v-col>
        <v-col cols="8">
      ...
      <v-row>
        <v-col cols="4">
          <p class="small">
            If they cared for children, what was the age range? (Optional)
          </p>
        </v-col>
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.ageofChildrenCaredFor }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ formatDate(evaluation.startDate!, "LLLL d, yyyy") }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ formatDate(evaluation.endDate!, "LLLL d, yyyy") }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.referenceRelationship }}
          </p>
        </v-col>
      </v-row>
      <v-row>
...
export default defineComponent({
  ...
  computed: {
    evaluation(): Components.Schemas.WorkExperienceReferenceDetails {
      ...
      const referenceRelationshipDisplay =
        workReference400HoursRelationshipRadio.find(
          (value) =>
            value.value ===
            this.wizardStore.wizardData[
              this.wizardStore.wizardConfig.steps
      ...
      return {
        ...
        role: this.wizardStore.wizardData[
          this.wizardStore.wizardConfig.steps?.workExperience400HoursEvaluation
            ?.form?.inputs?.workExperience400HoursEvaluation?.id || ""
        ]?.role,
        ...
        referenceRelationship:
          referenceRelationshipDisplay as Components.Schemas.ReferenceRelationship,
        additionalComments:
          this.wizardStore.wizardData[
            this.wizardStore.wizardConfig.steps
              ?.workExperience400HoursEvaluation?.form?.inputs
