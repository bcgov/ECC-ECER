  ...
  >
    <template #content>
      ...
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.lengthOfAcquaintance }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="4">
          <p class="small">
            Have you observed the applicant working with young children?
          </p>
        </v-col>
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.workedWithChildren ? "Yes" : "No" }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.childInteractionObservations }}
          </p>
        </v-col>
      </v-row>
      <v-row>
        ...
        <v-col cols="8">
          <p class="small font-weight-bold">
            {{ evaluation.applicantTemperamentAssessment }}
          </p>
        </v-col>
      </v-row>
    </template>
...
export default defineComponent({
  ...
  computed: {
    evaluation(): Components.Schemas.CharacterReferenceEvaluation {
      ...
      return {
        referenceRelationship:
          referenceReferenceDisplay as Components.Schemas.ReferenceRelationship,
        ...
        lengthOfAcquaintance:
          lengthOfAcquaintanceDisplay as Components.Schemas.ReferenceKnownTime,
        workedWithChildren:
          this.wizardStore.wizardData[
            this.wizardStore?.wizardConfig?.steps?.referenceEvaluation?.form
              ?.inputs?.characterReferenceEvaluation?.id || ""
