  ...
  <v-container>
    ...
    <v-row>
      <v-col class="text-break">
        ...
        licensed child care facility in B.C. people must be certified by the ECE
        ...
        to learn more about the ECE Registry, certificate types, or status
        information.
        <p class="mt-3">
          To find an ECE, you can search by name or registration number. The
          registration number is on an ECEs certificate that's posted on the
          wall where they work.
        </p>
      </v-col>
    </v-row>
    <v-form ref="lookupForm" class="mt-10">
      <v-row>
        <v-col cols="12" sm="8" lg="4">
          <EceTextField
            v-model="lookupCertificationStore.firstName"
            label="First name"
          ></EceTextField>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" sm="8" lg="4">
          <EceTextField
            v-model="lookupCertificationStore.lastName"
            label="Last name"
          ></EceTextField>
        </v-col>
      </v-row>
    ...
    <v-row
      v-if="lookupCertificationStore.certificationSearchResults?.length === 0"
    >
      <v-col>
        ...
        <ul class="ml-10">
          <li>
            Name was entered incorrectly - check the spelling and that you're
            entering their legal name
          </li>
    ...
    <v-row
      v-else-if="
        lookupCertificationStore.certificationSearchResults &&
        lookupCertificationStore.certificationSearchResults?.length > 0
      "
    >
      <v-col>
        <h2>
        ...
        >
          <template #item.name="{ item }">
            <a href="#" @click.prevent="() => applicantClick(item)">
              {{ item.name }}
            </a>
          </template>
          <template #item.statusCode="{ item }">
            <span>
              {{
                `${item.statusCode}${item.hasConditions ? " with Terms and Conditions" : ""}`
              }}
            </span>
          </template>
...
export default defineComponent({
  ...
  methods: {
    customAtLeastOneRule() {
      return () =>
        !!(
          this.lookupCertificationStore.firstName &&
          this.lookupCertificationStore.firstName?.trim()
        ) ||
        !!(
          this.lookupCertificationStore.lastName &&
          this.lookupCertificationStore.lastName?.trim()
        ) ||
        !!(
          this.lookupCertificationStore.registrationNumber &&
          this.lookupCertificationStore.registrationNumber?.trim()
        );
    },
    async handleSubmit() {
      try {
        const { valid, errors } = await (
          this.$refs.lookupForm as VForm
        ).validate();
