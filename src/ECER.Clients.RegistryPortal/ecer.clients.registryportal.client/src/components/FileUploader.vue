  ...
  <v-row>
    <v-col>
      ...
      <p>
        ...
        documents, and PDFs. Max file size accepted is 10MB.
      </p>
      <Callout class="mt-3" type="warning">
        <div class="d-flex flex-column ga-3">
          <p v-if="selectedFiles.length >= maxNumberOfFiles">
            ...
            {{ maxNumberOfFiles }} file{{ maxNumberOfFiles > 1 ? "s" : "" }}.
          </p>
          <p v-else>
  ...
  computed: {
    ...
    fileTooLarge() {
      return this.selectedFiles.some(
        (file) => file.file.size > this.maxFileSizeInBytes,
      );
    },
    allFilesTooLarge() {
    ...
    handleFileUpload(event: Event) {
      ...
      if (files) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (
            this.selectedFiles.some((f: FileItem) => f.file.name === file.name)
          ) {
            fileErrors.push(
              "This file with this name has already been uploaded. ",
            );
          }
        ...
        const response = await uploadFile(
          ...
          (progressEvent: AxiosProgressEvent) => {
            const fileIndex = this.selectedFiles.findIndex(
              (f: FileItem) => f.fileId === selectedFile.fileId,
            );
    ...
    async removeFile(selectedFile: FileItem) {
      try {
        ...
        if (
          ...
          selectedFile.storageFolder === "temporary" &&
          this.deleteFileFromTempWhenRemoved
        ) {
