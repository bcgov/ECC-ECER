...
export default defineComponent({
  ...
  setup: async () => {
    ...
    if (userProfile !== null) {
      formStore.initializeForm({
        [profileInformationForm?.inputs?.legalFirstName?.id || ""]:
          userProfile.firstName,
        [profileInformationForm?.inputs?.legalLastName?.id || ""]:
          userProfile.lastName,
        [profileInformationForm?.inputs?.dateOfBirth?.id || ""]:
          userProfile.dateOfBirth,
        [profileInformationForm?.inputs?.addresses?.id || ""]: {
          [AddressType.RESIDENTIAL]:
            userProfile.residentialAddress || oidcAddress,
          [AddressType.MAILING]: userProfile.mailingAddress || oidcAddress,
        },
        [profileInformationForm?.inputs?.email?.id || ""]: userProfile.email,
        [profileInformationForm?.inputs?.legalMiddleName?.id || ""]:
          userProfile.middleName,
        [profileInformationForm?.inputs?.preferredName?.id || ""]:
          userProfile.preferredName,
        [profileInformationForm?.inputs?.alternateContactNumber?.id || ""]:
          userProfile.alternateContactPhone,
        [profileInformationForm?.inputs?.primaryContactNumber?.id || ""]:
          userProfile.phone,
      });
    } else {
      formStore.initializeForm({
        [profileInformationForm?.inputs?.legalFirstName?.id || ""]:
          oidcUserInfo.firstName,
        [profileInformationForm?.inputs?.legalLastName?.id || ""]:
          oidcUserInfo.lastName,
        [profileInformationForm?.inputs?.dateOfBirth?.id || ""]:
          oidcUserInfo.dateOfBirth,
        [profileInformationForm?.inputs?.addresses?.id || ""]: {
          [AddressType.RESIDENTIAL]: oidcAddress,
          [AddressType.MAILING]: oidcAddress,
        },
  ...
  methods: {
    async saveProfile() {
      const { valid } = await (this.$refs.profileForm as typeof EceForm).$refs[
        profileInformationForm.id
      ].validate();
      ...
      if (!valid) {
        ...
        const { error } = await putProfile({
          ...
          email:
            this.formStore.formData[
              profileInformationForm?.inputs?.email?.id || ""
            ],
          phone:
            this.formStore.formData[
              profileInformationForm?.inputs?.primaryContactNumber?.id || ""
