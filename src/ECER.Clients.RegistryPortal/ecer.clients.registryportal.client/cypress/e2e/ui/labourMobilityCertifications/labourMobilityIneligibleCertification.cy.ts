...
describe("Labour Mobility - Ineligible Certification Application", () => {
  it("should sucessfully verify an Ineligible LM Certification Application", () => {
    ...
    cy.contains("The province or territory you hold certification in is not eligible to be transferred to B.C.").should("be.visible");

    /** Northwest Territories */
    cy.get(selectors.transferEligibility.provinceDropDownList).should("exist").click({ force: true });
    cy.get(selectors.elementType.vListItem).contains("Northwest Territories").click();
    cy.contains("The province or territory you hold certification in is not eligible to be transferred to B.C.").should("be.visible");
