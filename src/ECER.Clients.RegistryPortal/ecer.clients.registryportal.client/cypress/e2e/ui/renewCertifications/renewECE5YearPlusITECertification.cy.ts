...
describe("Renew ECE 5 Year + ITE +SNE Certificate Application", () => {
  it("should sucessfully create a Renewal ECE 5 Year + ITE +SNE Application", () => {
    /** Dashboard */
    cy.get(selectors.dashboard.renew).click();

    /** Certification Type */
    cy.get(selectors.certificationType.iteCheckBox).check();
    cy.get(selectors.certificationType.sneCheckBox).check();
