...
describe("Managed Submitted ECE5Year Certification Add New WE Refrence", () => {
  it("should sucessfully create a New ECE 5 Year Application and add new WE reference", () => {
    ...
    cy.reload();
    cy.get(selectors.elementType.span_button)
      .contains("Manage Application")
      .click();
    cy.get(selectors.elementType.text_links)
      .contains("500 hours of work experience with reference")
      .click();
    ...
    cy.get('[aria-label="Email"]').type(
      "orkExperience_Reference@test.gov.bc.ca",
    );
    cy.get('[aria-label="Phone number (optional)"]').type("1234567890");
    cy.get('[aria-label="Work experience hours observed by reference"]').type(
      "500",
    );
    cy.get(selectors.elementType.span_button)
      .contains("Save new reference")
      .click();
  });
});
