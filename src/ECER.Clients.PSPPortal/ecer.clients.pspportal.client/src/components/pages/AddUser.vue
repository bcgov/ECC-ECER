    ...
    <PageContainer>
        ...
        <v-row>
            <v-col class="mt-4" cols="12">
                <EceForm ref="addUserFormRef" :form="inviteUserForm" :form-data="formStore.formData"
                    @updated-form-data="formStore.setFormData" />
                <v-row class="mt-10">
                    <v-col>
                        <div class="d-flex flex-row justify-start ga-2">
                            <v-btn rounded="lg" color="primary" :loading="loadingStore.isLoading('psp_user_add')"
                                @click="saveUser">Send invitation</v-btn>
                            <v-btn rounded="lg" color="primary" variant="outlined"
                                :loading="loadingStore.isLoading('psp_user_add')" @click="router.back()">Cancel</v-btn>
                        </div>
                    </v-col>
                </v-row>
            </v-col>
