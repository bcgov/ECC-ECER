    ...
    <template v-else>
        ...
        <v-col cols="12">
          <v-sheet color="support-surface-info " class="w-100 px-12 py-7">
            <div class="d-flex flex-column ga-3 text-support-border-info">
              ...
              <p>
                You may add a new user from your post-secondary institution.
              </p>
              <p>The user:</p>
              <ol class="ml-8">
                <li>
                  Will receive an email inviting them to login to this PSP
                  Portal.
                </li>
                <li>
                  Will require a Business BCeID account granted from your
                  institution in order to login.
                </li>
              </ol>
            </div>
            <v-btn
      ...
      <v-row>
        <v-col cols="12">
          <p class="font-weight-bold">
            The users shown below are registered for access to this portal for
        ...
        <v-row>
          <v-col cols="12">
            <div class="d-flex flex-column ga-3">
              <p>
                The Primary contact is the person that the ECE Registry would
                contact first, if there is a reason to do so.
              </p>
              <ul class="ml-8">
                <li>
                  Setting a user as the Primary contact user does not apply any
                  additional permissions to the user in this portal.
                </li>
                <li>
                  One active user must be indicated as your Primary contact for
                  your ECE programs.
                </li>
              </ul>
            </div>
      ...
      <template v-if="invitedUsers.length > 0">
        ...
        <v-row>
          <v-col cols="12">
            <p>
              New and reactivated users will show as “Invitation pending” until
              they log in to verify their account.
            </p>
          </v-col>
        </v-row>
        <v-row>
      ...
      <template v-if="inactiveUsers.length > 0">
        ...
        <v-row>
          <v-col cols="12">
            <p>
              Making a user active will send them a new invitation to login to
              the portal.
            </p>
          </v-col>
        </v-row>
  ...
  computed: {
    activeUsers(): PspUserListItem[] {
      return this.users.filter(
        (user: PspUserListItem) => user.accessToPortal === "Active",
      );
    },
    invitedUsers(): PspUserListItem[] {
      return this.users.filter(
        (user: PspUserListItem) => user.accessToPortal === "Invited",
      );
    },
