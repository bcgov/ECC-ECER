...
RUN dotnet restore "ECER.Clients.PSPPortal/ECER.Clients.PSPPortal.Server/ECER.Clients.PSPPortal.Server.csproj"  -r linux-x64 -p:PublishReadyToRun=true
...
# build publish
RUN dotnet publish "ECER.Clients.PSPPortal/ECER.Clients.PSPPortal.Server/ECER.Clients.PSPPortal.Server.csproj" -c Release -o /app/publish --no-restore  --self-contained -r linux-x64 -p:PublishReadyToRun=true
...
ENTRYPOINT ["dotnet", "ECER.Clients.PSPPortal.Server.dll"]
