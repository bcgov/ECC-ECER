...
internal sealed class BcscRegistrationIdentityService(
  ...
  ILogger logger) : IRegistrationIdentityService
{
  public async Task<string> Resolve(RegisterNewUserCommand command, CancellationToken cancellationToken)
  {
    ...
    if (matchedRegistrant != null)
    {
      ...
      matchedRegistrant.Profile.ResidentialAddress = registrantMapper.MapAddress(command.Profile.ResidentialAddress!);
      matchedRegistrant.Profile.MailingAddress = registrantMapper.MapAddress(command.Profile.MailingAddress!);
