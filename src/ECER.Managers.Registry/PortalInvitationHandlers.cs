  ...
  : IRequestHandler<PortalInvitationVerificationQuery, PortalInvitationVerificationQueryResult>
{
    ...
    PortalInvitation portalInvitation, CancellationToken cancellationToken)
  {
    ...
    var registrantResult =
      await registrantRepository.Query(new RegistrantQuery() { ByUserId = portalInvitation.ApplicantId },
        cancellationToken);
    ...
    var Certifications =
      await certificationRepository.Query(new UserCertificationQuery() { ByApplicantId = applicant.Id });

    var latestCertification =
      Certifications.FirstOrDefault(); // Get the first certification with the latest expiry date
